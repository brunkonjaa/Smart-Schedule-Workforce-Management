...
The pushed verification checkpoint is `8ed7c28c1b7f3e33377dc1012378676f31ee0931`, committed on 20 July 2026 at 18:21:34 +01:00. It includes WebSocket lifetime re-authorisation/tests, backend coverage thresholds and the PostgreSQL-backed GitHub Actions workflow. The local migrated run passes `91` tests across `14` suites, and the first remote backend workflow passed in 55 seconds. Screenshot evidence now reaches `140`, including a fresh hosted staff session, workplace and direct NodyChat views, the full migration chain, actual 200% Chrome zoom, local terminal coverage, rejected Time Off date order, focused invalid assignment-time feedback and the successful GitHub Actions job.
